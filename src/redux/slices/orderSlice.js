    ...
    async (data) => {
        try {
            ...
            toast.promise(response, {
                ...
                success: (resolvedPromise) => resolvedPromise?.data?.message || "Order created successfully",
    ...
    async (orderId) => {
        try {
            ...
            toast.promise(response, {
                ...
                error: (err) => err?.response?.data?.message || "Failed to cancel order",
            });
            const apiResponse = await response;
            return apiResponse;
        } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getAllOrders.fulfilled, (state, action) => {
                state.loading = false;
                const fetchedOrders = action?.payload?.data?.data || action?.payload?.data || [];
            ...
            .addCase(updateOrderStatus.fulfilled, (state, action) => {
                const updatedOrder = action?.payload?.data?.data || action?.payload?.data;
                if (updatedOrder) {
            ...
            .addCase(cancelOrder.fulfilled, (state, action) => {
                const cancelledOrder = action?.payload?.data?.data || action?.payload?.data;
                if (cancelledOrder) {
                    state.ordersData = state.ordersData.map((order) =>
